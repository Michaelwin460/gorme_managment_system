...
const DepAndCat = () => {
  ...
  return (
    <div className=" px-5 mt-4">
      <div
        className="dac-header d-flex justify-content-center p-2 mb-3"
      ...
      {/* Navbar */}
      <nav className="dac-navbar-custom ">
        <div
          className={`dac-navbar-section ${currentView === "department" ? "selected" : ""} `}
          onClick={() => setCurrentView("department")}
        >
          Department
        </div>
        <div
          className={`dac-navbar-section ${currentView === "category" ? "selected" : ""} `}
      ...
      >
        <h4 className="mb-0" style={{fontFamily: 'Montserrat'}}>
          {currentView === "department"
            ? "Department List"
      ...
      <div className="mt-3">
        <table className="table table-striped table-custom">
          ...
          <tbody className="text-center">
            {(currentView === "department" ? department : category).map(
              (item, index) => (
                <tr key={index}>
                  {editingRow === index ? (
                    <>
                      <td>
                        <input
                          ...
                          value={
                            editedData.name || editedData.category_name || ""
                          }
      ...
      <Link
        to={
          currentView === "department"
            ...
            : "/admin/add_equipment_category"
        }
        className="btn add-btn"
        
      >
        <i className="bi bi-plus m-3"></i>
        {currentView === "department"
          ? "Add Department"
